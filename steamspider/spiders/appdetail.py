...
class AppDetailSpider(Spider):
    ...
    def parse_item(self, response):
        ...
        for app_item in applist:
            ...
            yield Request(url=detail_url, callback=self.parse_detail,
                          meta={'app_id': app_id, 'name': name, 'released': released})
    ...
    def parse_detail(self, response):
        ...
        item['des'] = des.xpath('string(.)').extract_first().strip()
        # 轮播视频
        ...
        game_score = response.xpath(
            '//div[contains(@id,"game_area_metascore")]/div[contains(@class,"score")]/text()').extract_first().strip()
        if (game_score is None):
            item['game_area_metascore'] = 0
        else:
            item['game_area_metascore'] = game_score
